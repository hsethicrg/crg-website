...
jQuery( function( $ ) {
    ...
    $.validator.addMethod('validEmail', function (value, element) {
        ...
        var matchText = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
        return matchText.test(value);
    }, "Please enter a valid email.");
    $.validator.addMethod('officeEmailCheck', function (value, element) {
        if(this.optional(element)){
            return true;
        }
        ...
        if(comp != 'gmail' && comp != 'yahoo' && comp != 'aol' && comp != 'hotmail' && comp != 'outlook') {
            return true;
        }
    ...
    $('#learn_datawatch_form').validate({
        ...
        highlight: function (element) { // hightlight error inputs
           $(element).closest('.row').addClass('has-error'); // set error class to the control group
        },
        unhighlight: function (element) { // revert the change done by hightlight
            $(element).closest('.row').removeClass('has-error'); // set error class to the control group
        },
